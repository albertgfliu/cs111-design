...
long nwrites_to_crash = -1; //default is to not crashed
...
/*****************************************************************************
* OSPFS Crash Testing Design Project
*
* These two functions, ospfs_ioctl and check_crashed are two helper functions
* used to implement the crash testing design project.
*
* ospfs_ioctl is the handler invoked in place of the regular ioctl system
* call in the test directory
*
* check_crashed will tell us if the OSPFS file system is in a crashed state
*/

// ospfs_ioctl(inode, filp, command_arg, argument)
// This function takes any inode and filp input, ignoring them. They are just
// present so that we can issue an ioctl command. If command_arg is set to
// OSPFS_NWRITE, which I have defined in the header file as 999 and in the
// ioctl-nwrite program as also being 999, then we write the value of
// argument into nwrites_to_crash
// Returns: 0 upon success, -EINVAL if any argument other than OSPFS_NWRITE
// is specified
int
ospfs_ioctl(struct inode *inode, struct file *filp, unsigned int command_arg, unsigned long argument){
	if(command_arg == OSPFS_NWRITE){
		nwrites_to_crash = argument;
		eprintk("nwrites_to_crash set to: %ld\n", nwrites_to_crash);
		return 0;
	}
	eprintk("nwrites_to_crash not set\n");
	return -EINVAL;
}

// check_crashed()
// This function takes no argument. If
// Returns: 0 if the file system has crashed
// -1 if the system is not crashed
// or how many writes left to failure
...
//	     a "negative dentry", which has inode no attachment.
...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		if((OSPFS_DIRENTRY_SIZE * (f_pos - 2)) > (dir_oi->oi_size)){ //self-comment: we reached the end of a directory
			r = 1;		/* Fix me! */ //self-comment: fixed?
			break;		/* Fix me! */ //self-comment: fixed?
		}
		...
		//self-comment: get a pointer to next entry (od)
		od = ospfs_inode_data(dir_oi, OSPFS_DIRENTRY_SIZE * (f_pos - 2));
		entry_oi = ospfs_inode(od->od_ino);
		
		//self-comment: ignore if inode is NULL or zero
...
{
	/* EXERCISE: Your code here */
	void *freemap_blk_ptr = ospfs_block(OSPFS_FREEMAP_BLK);
	uint32_t i;
	for(i = OSPFS_FREEMAP_BLK; i < ospfs_super->os_nblocks; i++)
		if(bitvector_test(freemap_blk_ptr, i)) {
			bitvector_clear(freemap_blk_ptr, i);
			return i;
		}
	return 0;
}
...
free_block(uint32_t blockno)
{
	/* EXERCISE: Your code here */
	void *freemap_blk_ptr = ospfs_block(OSPFS_FREEMAP_BLK);
	bitvector_set(freemap_blk_ptr, blockno);
}
...
{
	// Your code here.
	int32_t retval = (b < OSPFS_NDIRECT + OSPFS_NINDIRECT) ? -1 : 0;
	return retval;
}
...
indir_index(uint32_t b)
{
	// Your code here.
	int32_t retval = -2;
	if(b < OSPFS_NDIRECT) //self-comment: if inside direct block
		retval = -1;
	else if(b < OSPFS_NDIRECT + OSPFS_NINDIRECT) //self-comment: if inside indirect block
		retval = 0;
	else if (indir2_index(b) == 0) //self-comment: if inside doubly indirect block
		retval = (b - (OSPFS_NDIRECT + OSPFS_NINDIRECT)) / OSPFS_NINDIRECT; //self-comment: offset of relevant indirect block within doubly indirect block (change this back if doesn't work?)
	return retval;
}
...
direct_index(uint32_t b)
{
	...
	if(b < OSPFS_NDIRECT)
		...
		retval = (b - OSPFS_NDIRECT);
	else
		retval = (b - OSPFS_NDIRECT) % OSPFS_NINDIRECT;
...
add_block(ospfs_inode_t *oi)
{
	...
	if(n >= OSPFS_MAXFILEBLKS){ //self-comment: if file is already at maximum (or even larger!)
	...
	else if(indir2_index(n) == 0){ //use doubly indirect block
		if(oi->oi_indirect2 == 0){ //allocate doubly indirect block if not allocated
		...
		if(indir2_block[indir_index(n)] == 0){
			allocated[1] = allocate_block();
			if(allocated[1] == 0){ //couldn't allocate indirect block inside indirect2 block
				if(allocated[0] != 0){ //if we allocated an indirect2 block, undo everything
			...
			indir2_block[indir_index(n)] = allocated[1]; //create a pointer to the allocated indirect block inside the indirect2 block
		}

		indir_block = ((uint32_t *)ospfs_block(indir2_block[indir_index(n)]));
		indir_block[direct_index(n)] = allocate_block();

		if(indir_block[direct_index(n)] == 0){ //if couldn't allocate, undo all allocations from before
			if(allocated[1] != 0){ //undo indir allocation
				free_block(allocated[1]);
				indir2_block[indir_index(n)] = 0;
			}
			if(allocated[0] != 0){ //undo indir2 allocation
				free_block(allocated[0]);
				oi->oi_indirect2 = 0;
			}
			return -ENOSPC;
		}
		dir_block = indir_block[direct_index(n)];
		memset((uint32_t *)ospfs_block(dir_block), 0, OSPFS_BLKSIZE);
	}
	else if(indir_index(n) == 0){ //use indirect block
		if(oi->oi_indirect == 0){ //allocate indirect block if not allocated
			allocated[0] = allocate_block();
			if(allocated[0] == 0)
				return -ENOSPC;
		...
		//allocate block for indirect block to point to
		allocated[1] = allocate_block();
		if(allocated[1] == 0){
			if(allocated[0] != 0){ //free the allocated indirect block if we allocated one, undoing everything
				free_block(allocated[0]);
				oi->oi_indirect = 0;
			}
			oi->oi_indirect = 0;
...
remove_block(ospfs_inode_t *oi)
{
	...
	if(indir2_index(n - 1) == 0){ //if we use a doubly indirect block
		...
		if(direct_index(n - 1) == 0){
			free_block(indir2_block[indir_index(n - 1)]);
			indir2_block[indir_index(n - 1)] = 0;
		}
		...
		if(direct_index(n - 1) == 0){ //that is, direct block pointer we were looking at was first element of indirect block array of pointers
	...
	else{ //block to remove is within direct blocks
		free_block(oi->oi_direct[direct_index(n - 1)]); //free the block
		oi->oi_direct[direct_index(n - 1)] = 0; //set the block pointer to zero
		oi->oi_size = (n - 1) * OSPFS_BLKSIZE; //calculate new size
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	...
	while (ospfs_size2nblocks(oi->oi_size) < ospfs_size2nblocks(new_size)) {
		...
		if(r == -ENOSPC){
			new_size = old_size;
			break; //stop growing it and go on to removing blocks
		}
	}
	while (ospfs_size2nblocks(oi->oi_size) > ospfs_size2nblocks(new_size)) {
	        /* EXERCISE: Your code here */
		...
		//Case A to getting here: we new_size is smaller than old_size
		//Case B to getting here: we ran out of size, so we're reducing file back to its original size
		r = remove_block(oi);
		if(r == -EIO){
			return -EIO;
		}
	...
	//if we got to this point, we successfully changed the file's size

	oi->oi_size = new_size; //"(The value that the final add_block or remove_block set it to is probably not correct)."
	return r;
}
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		//if we're reading anywhere that's not the start of a block, this is less than OSPFS_BLKSIZE
		n = OSPFS_BLKSIZE - (*f_pos % OSPFS_BLKSIZE);

		//if we end anywhere that's not the end of the block, we need to reduce accordingly
		if(n > (count - amount)){
			n = (count - amount);
		}
		if(copy_to_user(buffer, data, n) < 0){//self-comment: int copy_to_user(void *dst, const void *src, unsigned int size)
			retval = -EFAULT;
			goto done;
		} 
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		//if we start writing in the middle of a block
		n = OSPFS_BLKSIZE - (*f_pos % OSPFS_BLKSIZE);

		//if we don't want to write entirely to the end of a block
		if(n > count - amount)
			n = count - amount;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	for(offset = 0; offset < dir_oi->oi_size; offset += OSPFS_DIRENTRY_SIZE){
		...
		if(od->od_ino == 0) //if directory entry is empty
			return od;
	}
	//otherwise, no empty entry was found so we need to add a block to the directory

	//r = change_size(dir_oi, ospfs_size2nblocks(dir_oi->oi_size)*OSPFS_BLKSIZE + OSPFS_BLKSIZE);
	r = add_block(dir_oi);
	...
	dir_oi->oi_size = dir_oi->oi_size + OSPFS_BLKSIZE;
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	ospfs_inode_t *dir_oi; //directory inode
	ospfs_inode_t *src_dir_oi; //source directory inode for the file we are hardlinking to
	ospfs_direntry_t *new_direntry; //new directory entry
	...
	if(find_direntry(dir_oi, dst_dentry->d_name.name, dst_dentry->d_name.len) != 0) //if a link with the same name already exists in the directory we are creating it in
		return -EEXIST;

	new_direntry = create_blank_direntry(dir_oi);

	//this comes from comments in create_blank_direntry
...
//function given by Tuan during friday discussion, returns index of a free inode in the inode table
//we will use this function in ospfs_create
uint32_t 
find_free_inode(void) {
	...
	return 0; //no free inodes were found
}
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	if(find_direntry(dir_oi, dentry->d_name.name, dentry->d_name.len) != 0){ //if direntry with same name already exists
		return -EEXIST;
	}
	...
	ino_temp->oi_size = 0;
	ino_temp->oi_ftype = OSPFS_FTYPE_REG;
	ino_temp->oi_nlink = 1;
	ino_temp->oi_mode = mode;
	
	direntry_temp = create_blank_direntry(dir_oi);

	//this comes from comments in create_blank_direntry
	if (IS_ERR(direntry_temp)){
		return PTR_ERR(direntry_temp);
	}

	//set directory entry values
	direntry_temp->od_ino = entry_ino;
	memcpy(direntry_temp->od_name, dentry->d_name.name, dentry->d_name.len);
	direntry_temp->od_name[dentry->d_name.len] = '\0';
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	//grab an inode to store the new symlink in
	entry_ino = find_free_inode();
	if(entry_ino == 0)
		return -ENOSPC;
	...
	//set the directory entry values of where the new symlink is going to be stored and initialize the values
	direntry_temp = create_blank_direntry(dir_oi);
	if(IS_ERR(direntry_temp)){
		return PTR_ERR(direntry_temp);
	}
...
//Did not implement handling a conditional symlink
static void *
